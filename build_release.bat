@echo off
REM Build a release version of Handy.
REM Release builds use windows_subsystem = "windows", so no CMD window appears.
REM Output: src-tauri\target\release\handy.exe
REM         src-tauri\target\release\bundle\nsis\Handy_0.8.3_x64-setup.exe
