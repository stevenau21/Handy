...
// 1. Write VoiceCommandsSettings.tsx
...
fs.writeFileSync('f:\\projects\\Handy\\src\\components\\settings\\commands\\VoiceCommandsSettings.tsx', tsx);
console.log('VoiceCommandsSettings.tsx written');

// 2. Patch translation.json to add openFolder key
const trPath = 'f:\\projects\\Handy\\src\\i18n\\locales\\en\\translation.json';
let tr = fs.readFileSync(trPath, 'utf8');
if (!tr.includes('"openFolder"')) {
  tr = tr.replace('"sendMessage": "Send Message"', '"openFolder": "Open Folder",\n      "sendMessage": "Send Message"');
...
// 3. Add "open downloads" to user settings
const settingsPath = process.env.APPDATA + '\\com.pais.handy\\settings_store.json';
const settings = JSON.parse(fs.readFileSync(settingsPath));
const existing = settings.settings.voice_commands || [];
const newCmd = { id: 'cmd_open_downloads', phrase: 'open downloads', action_type: 'open_folder', action_payload: 'C:\\Users\\steve\\Downloads', enabled: true };
if (!existing.some(c => c.id === newCmd.id)) {
  existing.push(newCmd);
}
settings.settings.voice_commands = existing;
fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2));
console.log('"open downloads" command added');
console.log('Total commands:', existing.length);
existing.forEach(c => console.log(c.phrase, '->', c.action_type, '->', c.action_payload));